...
var Terminal = Terminal || function(containerId) {
  ...
  var worker_ = new Worker('static/dist/js/worker.js');
  worker_.onmessage = function(e) {
    ...
    if (data.msg) {
      terminal_output('<div>' + data.msg + '</div>');
    }
  };
  ...
  container_.insertAdjacentHTML('beforeEnd',
      ['<terminal_output></terminal_output>',
       ...
       '<div class="prompt">You ➜</div><div><input class="cmdline" autofocus /></div>',
       '</div>'].join(''));
  var cmdLine_ = container_.querySelector('#input-line .cmdline');
  var terminal_output_ = container_.querySelector('terminal_output');
  var interlace_ = document.querySelector('.interlace');
  var bell_ = new Sound(false);
  bell_.load('static/dist/sounds/beep.mp3', false);

  // Hackery to resize the interlace background image as the container grows.
  terminal_output_.addEventListener('DOMSubtreeModified', function(e) {
  ...
  terminal_output_.addEventListener('click', function(e) {
    var el = e.target;
  ...
  function keyboardShortcutHandler_(e) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.keyCode == 83) { // crtl+s
      ...
      terminal_output('<div>Screen flicker: ' +
  ...
  function processNewCommand_(e) {
    ...
    if (e.keyCode == 9) { // Tab
      ...
      // Duplicate current input and append to terminal_output section.
      ...
      input.readOnly = true;
      terminal_output_.appendChild(line);
      ...
      switch (cmd) {
        case '3d':
          clear_(this);
          terminal_output('Hold on to your butts!');
          toggle3DView_();
          break;
        case 'cat':
          ...
          if (!fileName) {
            terminal_output('usage: ' + cmd + ' filename');
            break;
          }

          read_(cmd, fileName, function(result) {
            terminal_output('<pre>' + result + '</pre>');
          });
        ...
        case 'clear':
          ...
          return;
        case 'date':
          terminal_output((new Date()).toLocaleString());
          break;
        case 'exit':
          if (is3D_) {
            toggle3DView_();
          ...
          break;
        case 'help':
          terminal_output('<div class="ls-files">' + CMDS_.join('<br>') + '</div>');
          terminal_output('<p>Add files by dragging them from your desktop.</p>');
          break;
        case 'install':
          // Check is installed.
          if (window.chrome && window.chrome.app) {
            if (!window.chrome.app.isInstalled) {
              ...
              terminal_output('This app is already installed.');
            }
          }
          break;
        case 'ls':
          ls_(function(entries) {
            if (entries.length) {
              ...
              html.push('</div>');
              terminal_output(html.join(''));
            }
          });
          break;
        case 'pwd':
          terminal_output(cwd_.fullPath);
          break;
        case 'cd':
          var dest = args.join(' ') || '/';
          ...
          cwd_.getDirectory(dest, {}, function(dirEntry) {
            cwd_ = dirEntry;
            terminal_output('<div>' + dirEntry.fullPath + '</div>');
        ...
        case 'mkdir':
          ...
          if (!args.length) {
            terminal_output('usage: ' + cmd + ' [-p] directory<br>');
            break;
          }
        ...
        case 'mv':
          ...
          if (!src || !dest) {
            terminal_output(['usage: ', cmd, ' source target<br>',
                   '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;', cmd,
          ...
          args.forEach(function(fileName, i) {
            cwd_.getFile(fileName, {}, function(fileEntry) {
              ...
              if (recursive && e.code == FileError.TYPE_MISMATCH_ERR) {
                ...
                terminal_output(cmd + ': ' + fileName + ': is a directory<br>');
              } else {
                errorHandler_(e);
              }
            });
            ...
            cwd_.getDirectory(dirName, {}, function(dirEntry) {
              dirEntry.remove(function() {
                ...
                if (e.code == FileError.INVALID_MODIFICATION_ERR) {
                  terminal_output(cmd + ': ' + dirName + ': Directory not empty<br>');
                } else {
                  errorHandler_(e);
                }
              });
        ...
        case 'sudo':
          ...
          timer_ = setInterval(function() {
            terminal_output("<div>YOU DIDN'T SAY THE MAGIC WORD!</div>");
          }, 100);
          break;
        case 'theme':
          var theme = args.join(' ');
          if (!theme) {
            terminal_output(['usage: ', cmd, ' ' + THEMES_.join(',')].join(''));
          } else {
            if (THEMES_.indexOf(theme) != -1) {
              setTheme_(theme);
            } else {
              terminal_output('Error - Unrecognized theme used');
            }
          }
          break;
        case 'version':
        case 'ver':
          terminal_output(VERSION_);
          break;
        case 'wget':
          var url = args[0];
          if (!url) {
            terminal_output(['usage: ', cmd, ' missing URL'].join(''));
            break;
          ...
          xhr.onload = function(e) {
            if (this.status == 200 && this.readyState == 4) {
              terminal_output('<textarea>' + this.response + '</textarea>');
            } else {
              terminal_output('ERROR: ' + this.status + ' ' + this.statusText);
            }
          };
          xhr.onerror = function(e) {
            terminal_output('ERROR: ' + this.status + ' ' + this.statusText);
            terminal_output('Could not fetch ' + url);
          };
          xhr.open('GET', url, true);
          xhr.send();
          break;
        case 'who':
          terminal_output(document.title +
                 ' - By: Eric Bidelman &lt;ericbidelman@chromium.org&gt;');
          break;
        default:
          if (cmd) {
            terminal_output(cmd + ': command not found');
          }
      };
  ...
  function formatColumns_(entries) {
    ...
    // If we have 3 or less entries, shorten the terminal_output container's height.
  ...
  function invalidOpForEntryType_(e, cmd, dest) {
    if (e.code == FileError.NOT_FOUND_ERR) {
      terminal_output(cmd + ': ' + dest + ': No such file or directory<br>');
    } else if (e.code == FileError.INVALID_STATE_ERR) {
      terminal_output(cmd + ': ' + dest + ': Not a directory<br>');
    } else if (e.code == FileError.INVALID_MODIFICATION_ERR) {
      terminal_output(cmd + ': ' + dest + ': File already exists<br>');
    } else {
      errorHandler_(e);
    }
  }
  ...
  function errorHandler_(e) {
    ...
    terminal_output('<div>Error: ' + msg + '</div>');
  }
  ...
  function open_(cmd, path, successCallback) {
    ...
    cwd_.getFile(path, {}, successCallback, function(e) {
      if (e.code == FileError.NOT_FOUND_ERR) {
        terminal_output(cmd + ': ' + path + ': No such file or directory<br>');
      }
    });
  }
  ...
  function read_(cmd, path, successCallback) {
    ...
    cwd_.getFile(path, {}, function(fileEntry) {
      ...
      if (e.code == FileError.INVALID_STATE_ERR) {
        terminal_output(cmd + ': ' + path + ': is a directory<br>');
  ...
  function terminal_output(html) {
    terminal_output_.insertAdjacentHTML('beforeEnd', html);
    //terminal_output_.scrollIntoView();
    cmdLine_.scrollIntoView();
  }

  return {
    initFS: function(persistent, size) {
      terminal_output('<div>Welcome to ' + document.title +
             '! (v' + VERSION_ + ')</div>');
      terminal_output((new Date()).toLocaleString());
      terminal_output('<p>Documentation: type "help"</p>');

      if (!!!window.requestFileSystem) {
        terminal_output('<div>Sorry! The FileSystem APIs are not available in your browser.</div>');
        return;
      }
      ...
      window.requestFileSystem(type, size, function(filesystem) {
        ...
        cwd_.getDirectory('testquotaforfsfolder', {create: true}, function(dirEntry) {
          ...
          if (e.code == FileError.QUOTA_EXCEEDED_ERR) {
            terminal_output('ERROR: Write access to the FileSystem is unavailable.<br>');
            terminal_output('Type "install" or run Chrome with the --unlimited-quota-for-files flag.');
          } else {
            errorHandler_(e);
          }
        });
